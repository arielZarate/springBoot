...
import com.arielzarate.infraestructure.persistence.model.AddressEntity;
...
public class ClientMapper {
   ...
   public AddressEntity mapAddressesToEntity(Address address) {
        ...
        addressEntity.setStreet(address.getStreet().trim());
        addressEntity.setCity(address.getCity().trim());
        addressEntity.setState(address.getState().trim());
        addressEntity.setPostalCode(address.getPostalCode().trim());
    ...
    /**
     * for list of addresses, but in this case we have only one address per client, so we can use the method above
...
//            address.setPostalCode(addressEntity.getPostalCode().trim());
//            return address;
//        }).toList();
//    }
...
//            addressClientEntity.setPostalCode(address.getPostalCode().trim());
